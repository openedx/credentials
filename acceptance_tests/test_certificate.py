...
class RenderCredentialTests(LoginMixin, WebAppTest, CredentialsApiMixin):
    ...
    def setUp(self):
        ...
        self.data = self._create_credential()
        self._set_credential_status_awarded(self.data)

    def test_student_dashboard_with_certificate(self):
        """ Render the credentials link from student dashboard.
    ...
    def test_student_dashboard_without_certificate(self):
        """ If student has no credentials or with revoked status then it .
        will not apppear on dashboard. After successful login user will redirect
        towards the dashboard page. here search the certificate link and load the
        credential in a browser.
        """
        self._set_credential_status_revoked(self.data)
        self.login_with_lms()
        lms = LMSDashboardPage(self.browser).wait_for_page()
        lms.is_browser_on_page()
