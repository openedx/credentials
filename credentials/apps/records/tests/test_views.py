...
class RecordsViewTests(SiteMixin, TestCase):
    ...
    def _render_records(self, program_data=None, status_code=200):
        ...
        with patch(
            "credentials.apps.records.views.RecordsView._get_programs"
        ) as get_programs:
    ...
    def test_normal_access(self):
        ...
        self.assert_matching_template_origin(
            actual_child_templates["footer"], "_footer.html"
        )
        self.assert_matching_template_origin(
            actual_child_templates["header"], "_header.html"
        )
        self.assert_matching_template_origin(
            actual_child_templates["masquerade"], "_masquerade.html"
        )

    def test_xss(self):
        """ Verify that the view protects against xss in translations. """
        response = self._render_records(
            [{"name": "<xss>", "partner": "XSS", "uuid": "uuid",},]
        )
    ...
    def test_multiple_programs(self):
        ...
        new_course_cert = CourseCertificateFactory.create(
            course_id=new_course_run.key, site=self.site
        )
        new_program_cert = ProgramCertificateFactory.create(
            program_uuid=new_program.uuid, site=self.site
        )
...
class ProgramListingViewTests(SiteMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.user = UserFactory(
            username=self.MOCK_USER_DATA["username"], is_superuser=True
        )
        self.orgs = [
            OrganizationFactory.create(name=name, site=self.site)
            for name in ["TestOrg1", "TestOrg2"]
    ...
    def test_normal_access(self):
        ...
        self.assertNotIn(
            "masquerade", actual_child_templates
...
class ProgramRecordViewTests(SiteMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.program_content_type = ContentType.objects.get(
            app_label="credentials", model="programcertificate"
        )
        ...
        self.orgs = [
            OrganizationFactory(name=name, site=self.site) for name in self.org_names
        ]
        self.program = ProgramFactory(
            course_runs=self.course_runs,
            authoring_organizations=self.orgs,
    ...
    def _render_program_record(self, record_data=None, status_code=200):
        ...
        with patch(
            "credentials.apps.records.views.ProgramRecordView._get_record"
        ) as get_record:
            get_record.return_value = record_data
            response = self.client.get(
                reverse("records:private_programs", kwargs={"uuid": uuid.uuid4().hex})
            )
            self.assertEqual(response.status_code, status_code)
    ...
    def test_anonymous_access_public(self):
        ...
        self.client.logout()
        response = self.client.get(
            reverse("records:public_programs", kwargs={"uuid": self.pcr.uuid.hex})
        )
    ...
    def test_access_to_empty_record(self, is_superuser):
        ...
        response = self.client.get(
            reverse("records:private_programs", kwargs={"uuid": self.program.uuid.hex})
        )
    ...
    def test_future_visible_date_not_shown(self):
        ...
        self.assertEqual(len(grades), 1)
        self.assertEqual(
            grades[0]["course_id"], self.course_runs[0].key
        )  # 0 instead of 1 now that 1 is in future
        self.assertEqual(
            grades[0]["issue_date"], self.user_credentials[0].created.isoformat()
        )

    @ddt.data(
        ("9999-01-01T01:01:01Z", False), ("1970-01-01T01:01:01Z", True), (None, True),
    )
    @ddt.unpack
    def test_program_visible_date(self, date, completed):
        ...
        if date:
            UserCredentialAttributeFactory(
                user_credential=program_credential, name="visible_date", value=date,
            )
        response = self.client.get(
            reverse("records:private_programs", kwargs={"uuid": self.program.uuid.hex})
        )
        self.assertEqual(
            json.loads(response.context_data["record"])["program"]["completed"],
            completed,
        )
    ...
    def test_course_run_order(self):
        ...
        expected_course_run_keys = [
            course_run.key for course_run in [self.course_runs[1], new_course_run]
        ]
    ...
    def test_xss(self):
        ...
        response = self._render_program_record(
            {
                "name": "<xss>",
                "program": {"name": "<xss>", "school": "XSS School",},
                "uuid": "uuid",
            }
        )
...
class ProgramSendTests(SiteMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.user_credential = UserCredentialFactory(
            username=self.USERNAME, credential=self.pc
        )
        self.data = {"username": self.USERNAME, "pathway_id": self.pathway.id}
        self.url = reverse(
            "records:send_program", kwargs={"uuid": self.program.uuid.hex}
        )
    ...
    def test_from_address_set(self, mock_ace):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            mock_ace.send.call_args[0][0].options["from_address"],
    ...
    def test_no_full_name(self, mock_ace):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            mock_ace.send.call_args[0][0].context["user_full_name"], self.user.username
        )
    ...
    def test_email_content_complete(self):
        ...
        response = self.post()
        self.assertEqual(response.status_code, 200)
        public_record = ProgramCertRecord.objects.get(
        ...
        self.assertIn(
            ...
            + " would like to apply for credit in the "
            + self.pathway.name,
            message,
        )
    ...
    def test_email_content_incomplete(self):
        ...
        email = mail.outbox[0]
        self.assertIn(
            "has sent their partially completed program record for",
            str(email.message()),
        )
    ...
    def test_resend_email(self):
        ...
        response = self.post()
        self.assertEqual(response.status_code, 200)
        user_credit_pathway = UserCreditPathway.objects.get(
            user=self.user, pathway=self.pathway
        )
...
class ProgramRecordCsvViewTests(SiteMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.program_cert_record = ProgramCertRecordFactory.create(
            user=self.user, program=self.program
        )

    @patch("credentials.apps.records.views.SegmentClient", autospec=True)
    def test_404s_with_no_program_cert_record(
        self, segment_client
    ...
    def _render_page(self, page):
        ...
        if page == "records":
            ...
            with patch(
                ...
                response = self.client.get(
                    reverse(
                        "records:private_programs", kwargs={"uuid": uuid.uuid4().hex}
                    )
                )
