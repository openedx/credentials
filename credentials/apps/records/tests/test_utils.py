...
class UpdatedProgramEmailTests(SiteMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse(
            "records:share_program", kwargs={"uuid": self.program.uuid.hex}
        )
    ...
    def test_send_updated_email_when_program_finished(self):
        ...
        UserCreditPathwayFactory(
            user=self.user, pathway=self.pathway, status=UserCreditPathwayStatus.SENT
        )
        ...
        email = mail.outbox[0]
        record_path = reverse(
            "records:public_programs", kwargs={"uuid": self.pcr.uuid.hex}
        )
...
class MasqueradingAuthorizedTests(TestCase):
    ...
    def test_default_authorization(self):
        ...
        self.assertEqual(
            masquerading_authorized(self.staff_user, self.staff_user), False
        )
        self.assertEqual(
            masquerading_authorized(self.staff_user, self.superuser), False
        )
    ...
    @override_settings(
        HIJACK_AUTHORIZE_STAFF=False, HIJACK_AUTHORIZE_STAFF_TO_HIJACK_STAFF=False
    )
    ...
    @override_settings(
        HIJACK_AUTHORIZE_STAFF=True, HIJACK_AUTHORIZE_STAFF_TO_HIJACK_STAFF=True
    )
    def test_full_staff_authorization(self):
        ...
        self.assertEqual(masquerading_authorized(self.staff_user, self.user), True)
        self.assertEqual(
            masquerading_authorized(self.staff_user, self.staff_user), True
        )
        self.assertEqual(
            masquerading_authorized(self.staff_user, self.superuser), False
        )
