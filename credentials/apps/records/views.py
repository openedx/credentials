...
from credentials.apps.records.models import (
    ProgramCertRecord,
    UserCreditPathway,
    UserGrade,
)
...
class ConditionallyRequireLoginMixin(AccessMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(ConditionallyRequireLoginMixin, self).dispatch(
            request, *args, **kwargs
        )
...
def get_record_data(user, program_uuid, site, platform_name=None):
    ...
    user_credit_pathways = (
        ...
        .filter(user=user, pathway__in=program_pathways_set)
        .all()
    )
    ...
    pathways = [
        (pathway, user_credit_pathways_dict.setdefault(pathway, ""))
    ...
    for course_grade in course_grades:
        ...
        if user_credential is not None:
            ...
            last_updated = max(
                filter(None, [visible_date, course_grade.modified, last_updated])
            )
...
class RecordsListBaseView(
    LoginRequiredMixin, RecordsEnabledMixin, TemplateView, ThemeViewMixin
):
    ...
    def _course_credentials_to_course_runs(self, course_credentials):
        ...
        course_credential_ids = [
            x.credential_id
            for x in course_credentials
            if x.status == UserCredential.AWARDED
        ]
        course_certificates = CourseCertificate.objects.filter(
            id__in=course_credential_ids, site=self.request.site
        )
        course_run_keys = map(
            lambda course_certificate: course_certificate.course_id, course_certificates
        )
        return CourseRun.objects.filter(key__in=course_run_keys)

    def _programs_context(
        self, include_empty_programs=False, include_retired_programs=False
    ):
        ...
        course_runs = frozenset(
            self._course_credentials_to_course_runs(course_credentials)
        )
        course_filters = (
            {} if include_empty_programs else {"course_runs__in": course_runs}
        )
        ...
        program_credential_ids = map(
            lambda program_credential: program_credential.credential_id,
            program_credentials,
        )
        program_certificates = ProgramCertificate.objects.filter(
            id__in=program_credential_ids, site=self.request.site
        )
        completed_program_uuids = frozenset(
            program_certificate.program_uuid
        ...
        return [
            {
                ...
                "empty": not bool(
                    course_runs.intersection(frozenset(program.course_runs.all()))
                ),
            }
        ...
        context.update(
            {
                ...
                "icons_template": self.try_select_theme_template(
                    ["credentials/records.html"]
                ),
            }
        )
...
class RecordsView(RecordsListBaseView):
    def _get_programs(self):
        return self._programs_context(
            include_empty_programs=False, include_retired_programs=True
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["child_templates"]["masquerade"] = self.select_theme_template(
            ["_masquerade.html"]
        )
        ...
        context["program_help"] = _(
            "A program record is created once you have earned at least one "
...
class ProgramListingView(RecordsListBaseView):
    def _get_programs(self):
        return self._programs_context(
            include_empty_programs=True, include_retired_programs=False
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["program_help"] = _(
            "The following is a list of all active programs for which program records "
            "are being generated."
        )
...
class ProgramRecordView(
    ConditionallyRequireLoginMixin, RecordsEnabledMixin, TemplateView, ThemeViewMixin
):
    ...
    def get_context_data(self, **kwargs):
        ...
        site_configuration = self.request.site.siteconfiguration
        records_help_url = (
            site_configuration.records_help_url if site_configuration else ""
        )
...
@method_decorator(
    ratelimit(key="user", rate=RECORDS_RATE_LIMIT, method="POST", block=True),
    name="dispatch",
)
class ProgramSendView(LoginRequiredMixin, RecordsEnabledMixin, View):
    ...
    def post(self, request, **kwargs):
        ...
        public_record, _ = ProgramCertRecord.objects.get_or_create(
            user=user, program=program
        )

        # Make sure we haven't already sent an email with a 'sent' status
        if UserCreditPathway.objects.filter(
            user=user, pathway=pathway, status=UserCreditPathwayStatus.SENT
        ).exists():
            return JsonResponse({"error": "Pathway email already sent"}, status=400)

        record_path = reverse(
            "records:public_programs", kwargs={"uuid": public_record.uuid.hex}
        )
...
class ProgramRecordCreationView(LoginRequiredMixin, RecordsEnabledMixin, View):
    ...
    def post(self, request, **kwargs):
        ...
        program_uuid = kwargs["uuid"]
        program = get_object_or_404(Program, uuid=program_uuid, site=request.site)
        pcr, created = ProgramCertRecord.objects.get_or_create(
            user=user, program=program
        )
        status_code = 201 if created else 200

        url = request.build_absolute_uri(
            reverse("records:public_programs", kwargs={"uuid": pcr.uuid.hex})
        )
...
class ProgramRecordCsvView(RecordsEnabledMixin, View):
    ...
    class SegmentHttpResponse(HttpResponse):
        def __init__(self, *args, **kwargs):
            ...
            super(ProgramRecordCsvView.SegmentHttpResponse, self).__init__(
                *args, **kwargs
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        program_cert_record = get_object_or_404(
            ProgramCertRecord, uuid=kwargs.get("uuid")
        )
        ...
        writer = csv.DictWriter(
            string_io, record["grades"][0].keys(), quoting=csv.QUOTE_ALL
        )
        writer.writeheader()
