        ...
        context.update(
            {
                ...
                ).format(platform_name=site_configuration.platform_name),
            }
        )
        return context
...
class RenderCredential(SocialMediaMixin, ThemeViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if len(organization_names) == 1:
            ...
            org_name_string = _("{series_of_orgs}, and {last_org}").format(
                series_of_orgs=", ".join(organization_names[:-1]),
                last_org=organization_names[-1],
            )

        user_data = user_credential.credential.site.siteconfiguration.get_user_api_data(
        ...
        context.update(
            {
                ...
                "render_language": content_language
                if content_language
    ...
    def get_credential_template(self):
        ...
        template_names += [
            "credentials/programs/{uuid}/certificate.html".format(
                uuid=credential_type.program_uuid
            ),
