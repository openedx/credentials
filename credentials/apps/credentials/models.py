...
OrganizationDetails = namedtuple(
    "OrganizationDetails",
    ("uuid", "key", "name", "display_name", "certificate_logo_image_url"),
)
ProgramDetails = namedtuple(
    "ProgramDetails",
    (
        "uuid",
        "title",
...
class ProgramCertificate(AbstractCertificate):
    ...
    program_uuid = models.UUIDField(
        db_index=True, null=False, blank=False, verbose_name=_("Program UUID")
    )
    user_credentials = GenericRelation(
        UserCredential,
        content_type_field="credential_content_type",
    ...
    def get_program_api_data(self):
        ...
        return self.site.siteconfiguration.get_program(
            self.program_uuid
        ...
        for organization in data["authoring_organizations"]:
            organizations.append(
                OrganizationDetails(
                    ...
                    certificate_logo_image_url=organization[
...
class UserCredentialAttribute(TimeStampedModel):
    ...
    user_credential = models.ForeignKey(
        UserCredential, related_name="attributes", on_delete=models.CASCADE
    )
    name = models.CharField(max_length=255)
