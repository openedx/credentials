...
class CredentialViewSetTests(SiteMixin, APITestCase):
    ...
    def serialize_user_credential(self, user_credential, many=False):
        ...
        return UserCredentialSerializer(
            user_credential, context={"request": request}, many=many
        ).data
    ...
    def assert_access_denied(self, user, method, path, data=None):
        ...
        response = getattr(self.client, method)(
            path, data=data, content_type=JSON_CONTENT_TYPE
        )
    ...
    def test_create(self):
        ...
        data = {
            ...
            "status": "awarded",
            "attributes": [
                {"name": expected_attribute_name, "value": expected_attribute_value,}
            ],
        }
        ...
        response = self.client.post(
            self.list_path, data=json.dumps(data), content_type=JSON_CONTENT_TYPE
        )
    ...
    def test_retrieve(self):
        ...
        credential = UserCredentialFactory(
            credential__site=self.site, username=self.user.username
        )
        path = reverse("api:v2:credentials-detail", kwargs={"uuid": credential.uuid})
    ...
    def test_list_status_filtering(self):
        ...
        awarded = UserCredentialFactory.create_batch(
            3, credential__site=self.site, status=UserCredential.AWARDED
        )
        revoked = UserCredentialFactory.create_batch(
            3, credential__site=self.site, status=UserCredential.REVOKED
        )
    ...
    def test_list_username_filtering(self):
        ...
        username = self.user.username
        expected = UserCredentialFactory.create_batch(
            3, credential__site=self.site, username=username
        )
    ...
    def test_list_program_uuid_filtering(self):
        ...
        response = self.client.get(
            self.list_path + "?program_uuid={}".format(program.uuid)
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data["results"],
    ...
    def test_list_type_filtering(self):
        ...
        self.assertEqual(
            response.data["results"],
            self.serialize_user_credential([program_cred], many=True),
        )
...
class GradeViewSetTests(SiteMixin, APITestCase):
    ...
    def serialize_user_grade(self, user_grade, many=False):
        ...
        request = APIRequestFactory(SERVER_NAME=self.site.domain).get("/")
        return UserGradeSerializer(
            user_grade, context={"request": request}, many=many
        ).data
    ...
    def test_authentication(self):
        ...
        self.client.logout()
        response = self.client.post(
            self.list_path, data=json.dumps(self.data), content_type=JSON_CONTENT_TYPE
        )
        self.assertEqual(response.status_code, 401)
    ...
    def test_create_with_empty_letter_grade(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.data, self.serialize_user_grade(UserGrade.objects.last())
        )
    ...
    def test_update(self, method):
        ...
        grade = UserGradeFactory(
            course_run=self.course_run, username=self.user.username, letter_grade="C",
        )
...
class UsernameReplacementViewTests(JwtMixin, APITestCase):
    ...
    def call_api(self, user, data):
        ...
        return self.client.post(
            self.url, data, **headers, content_type=JSON_CONTENT_TYPE
        )
    ...
    def test_existing_and_non_existing_users(self):
        ...
        existing_users = [
            {user.username: user.username + "_new"} for user in random_users
        ]
        non_existing_users = [
            {username: username + "_new"} for username in fake_usernames
        ]
