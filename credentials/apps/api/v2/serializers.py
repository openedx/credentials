...
class CredentialField(serializers.Field):
    ...
    def to_internal_value(self, data):
        ...
        if program_uuid:
            try:
                return ProgramCertificate.objects.get(
                    program_uuid=program_uuid, is_active=True
                )
            except ObjectDoesNotExist:
                msg = "No active ProgramCertificate exists for program [{}]".format(
                    program_uuid
                )
        ...
        if course_run_key:
            if self.read_only:
                try:
                    cert = CourseCertificate.objects.get(
                        course_id=course_run_key, site=site
                    )
                except ObjectDoesNotExist:
                    cert = None
            else:
            ...
            if cert is None or not cert.is_active:
                msg = "No active CourseCertificate exists for course run [{}]".format(
                    course_run_key
                )
                logger.exception(msg)
...
class UserCredentialCreationSerializer(serializers.ModelSerializer):
    ...
    def issue_credential(self, validated_data):
        ...
        return accreditor.issue_credential(
            credential, username, status=status, attributes=attributes, request=request
        )
...
class UserGradeSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        grade, _ = UserGrade.objects.update_or_create(
            username=username, course_run=course_run, defaults=validated_data,
        )
        return grade
